...
<title>${e(seoTitle)}</title><meta name="description" content="${e(settings.description)}">
<link rel="canonical" href="${e(settings.site_url)}/"><meta property="og:type" content="website"><meta property="og:title" content="${e(seoTitle)}"><meta property="og:description" content="${e(settings.description)}"><meta property="og:url" content="${e(settings.site_url)}/">
...
<script src="/theme.js"></script><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
...
<link rel="preload" as="image" href="${safePath(settings.hero_image)}" fetchpriority="high"><link rel="stylesheet" href="/styles.css"><script src="/app.js" defer></script>
</head><body>
<a class="skip-link" href="#main">Skip to content</a>
<div class="announcement" id="announcement" ${offer?'':'hidden'}><div class="announcement-inner"><span class="announcement-label" id="offer-label">${e(offer?.label)}</span><span id="offer-text">${e(offer?.text)}</span><a id="offer-link" href="${e(/^(#[a-zA-Z][\w-]*|https:\/\/)/.test(offer?.href||'')?offer.href:'#sacred-circuits')}">${e(offer?.cta)} ${icon('arrow')}</a><button class="icon-button offer-pause" id="offer-pause" aria-label="Pause offers" hidden>Ⅱ</button></div></div>
<header class="site-header"><div class="nav-wrap"><a class="brand" href="#" aria-label="Irisha Concierge home">${wordmark}</a><nav class="desktop-nav" aria-label="Main navigation"><a href="#sacred-circuits">Sacred Circuits</a><a href="#elite-fleet">Elite Fleet</a><a href="#holidays">Holidays</a><a href="#contact">Contact</a></nav><button class="button button-small nav-cta" data-contact>${icon('whatsapp')} Inquire on WhatsApp</button><button type="button" class="theme-toggle" data-theme-toggle aria-label="Appearance: System. Switch to light mode"><span aria-hidden="true" class="theme-symbol">◐</span><span class="theme-label">System</span></button><button class="menu-toggle icon-button" aria-controls="mobile-nav" aria-expanded="false" aria-label="Open navigation">${icon('menu')}</button></div><nav id="mobile-nav" class="mobile-nav" aria-label="Mobile navigation" hidden><a href="#sacred-circuits">Sacred Circuits</a><a href="#elite-fleet">Elite Fleet</a><a href="#holidays">Holidays</a><a href="#contact">Contact</a><button class="mobile-inquire" data-contact>Inquire on WhatsApp ${icon('whatsapp')}</button></nav></header>
...
<section class="section container services-section" id="holidays" aria-labelledby="services-title"><div class="section-heading split-heading"><div><p class="eyebrow">ONE CONCIERGE. ENDLESS POSSIBILITIES.</p><h2 id="services-title">Holidays &amp; travel<br><em>concierge services.</em></h2></div><p>From a change of scenery to a celebration.<br>Every detail, beautifully in place.</p></div><div class="bento-grid">${services.map((x,i)=>`<button class="service-card ${x.large?'service-large':''}" data-service="${i}"><span class="service-top">${icon(x.icon)}<span class="circle-arrow">${icon('diagonal')}</span></span><span class="service-copy"><span class="small-eyebrow">${e(x.eyebrow)}</span><strong>${e(x.title)}</strong><span class="service-description">${e(x.details || x.description)}</span></span>${x.large?`<span class="service-extra">A LITTLE WONDER. A WORLD OF POSSIBILITIES.</span>`:''}</button>`).join('')}</div></section>
<section class="contact-section" id="contact"><div class="contact-glow"></div><div class="container contact-inner"><span class="contact-star" aria-hidden="true">✧</span><p class="eyebrow">YOUR NEXT CHAPTER</p><h2>${e(settings.contact_heading).replace('with a conversation.','<br><em>with a conversation.</em>')}</h2><p>${e(settings.contact_description)}</p><button class="button" data-contact>${icon('whatsapp')} Let’s talk on WhatsApp ${icon('arrow')}</button><span class="contact-note">A personal conversation. A journey made for you.</span><noscript><p class="contact-note">Please enable JavaScript to use secure contact verification.</p></noscript><div id="contact-verification" class="contact-verification" role="region" aria-labelledby="verification-title" hidden>
...
await writeFile('dist/sitemap.xml',`<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"><url><loc>${e(settings.site_url)}/</loc></url></urlset>`);
