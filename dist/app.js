...
(() => {
    ...
    try {
      ...
      if(widgetId===undefined){
        ...
        widgetId=window.grecaptcha.render('captcha-widget',{
          sitekey:config.siteKey,theme:'dark',size:widgetSize,
  ...
  function updateOffers(advance = false) {
    ...
    link.href = /^(#[a-zA-Z][\w-]*|https:\/\/)/.test(offer.href) ? offer.href : '#sacred-circuits';
    if(offer.href.startsWith('https://')){link.target='_blank';link.rel='noopener noreferrer';}else{link.removeAttribute('target');link.removeAttribute('rel');}
  }
